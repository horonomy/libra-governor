...
| Admission Admit rate on cold-start tasks under the shipped default (`balanced`) policy | **0/2 (0%)** — both scenario 1 and scenario 9's initial preflights were real `Deny`s on the confidence floor alone | `daemon.log` grep, see README "Defects and gaps found" #1; raw logs at `/tmp/mvp3-gate-work/state-s1-balanced/daemon.log` and `state-s9-crash/daemon.log` (not committed — ephemeral test state; the finding is documented and reproducible via `run_gate_matrix.py`) |

...
//! 2. The daemon binary itself — **only** if `install`'s own marker file
//!    ([`crate::install_cmd::INSTALL_MARKER_FILE_NAME`], read *before*
//!    the state directory that holds it is removed — see step 3) says
//!    this exact binary path was installed by `libra-governor install`.
//!    A binary built by `cargo build`, installed by `cargo install` by
//!    hand, or found some other way, is never touched — there is no
//!    `PATH` scan and no name-based guess.
...
pub fn run(assume_yes: bool) {
    ...
    uninstall_binary(marker_binary_path.as_deref(), assume_yes);
...
/// Removes the daemon binary only if the install marker's recorded path
/// equals `current_exe()` — the binary running this very uninstall
/// command must be the one the marker names. Confirmation-gated the same
/// way as the state directory, since it is also a real, if less
/// consequential, deletion.
fn uninstall_binary(marker_binary_path: Option<&str>, assume_yes: bool) {
    ...
    let confirmed = assume_yes || confirm("Also remove the daemon binary", &current_exe);
    if !confirmed {
        println!(
            "libra-governor uninstall: leaving the binary at {} in place (not confirmed)",
            current_exe.display()
        );
        return;
    }
    match std::fs::remove_file(&current_exe) {
        Ok(()) => println!(
            "libra-governor uninstall: removed {}",
            current_exe.display()
        ),
        Err(e) => eprintln!(
